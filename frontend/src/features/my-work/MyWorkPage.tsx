...
import MyWorkEmptyState from './components/MyWorkEmptyState';
import MyWorkSkeleton from './components/MyWorkSkeleton';
import MyWorkToolbar from './components/MyWorkToolbar';
import TaskCard from '../boards/components/TaskCard';
import TaskDetailsModal from '../boards/modals/task-details';
      ...
      <div className="max-w-5xl mx-auto py-10 px-8 flex flex-col gap-8">
        ...
        <section>
          <MyWorkSummary
            ...
            onFilterClick={(filter: any) => updateParam("due", filter)}
          />
        </section>
        ...
        <section className="flex flex-col gap-6">
          <MyWorkToolbar
            search={searchParam}
            setSearch={(val: any) => updateParam("search", val)}
            status={statusParam}
            setStatus={(val: any) => updateParam("status", val)}
            priority={priorityParam}
            setPriority={(val: any) => updateParam("priority", val)}
            due={dueParam}
            setDue={(val: any) => updateParam("due", val)}
            sort={sortParam}
            setSort={(val: any) => updateParam("sort", val)}
            grouping={groupingParam}
            setGrouping={(val: any) => updateParam("grouping", val)}
          />
          ...
          {completedThisWeekTasks.length > 0 && (
            <div className="mt-8 pt-8 border-t border-brand-border">
              ...
              <div className="flex flex-col gap-3 opacity-75">
                {completedThisWeekTasks.map((task: any) => (
                  <TaskCard
                    key={task.id}
                    task={task}
                    columns={[]}
